...
object Hierarchy {
  ...
  final case class Node(idx: Int, elem1: Int, elem2: Int, distance: Double,
                        //cardinality: Int
                       )

  private final def nodeFromArray(idx: Int, arr: Array[Double]): Node =
    Node(idx, arr(0).toInt, arr(1).toInt, arr(2))

  def newBuilder(n: Int): HierarchyBuilder = new HierarchyBuilder(n)

  class HierarchyBuilder private[Hierarchy] (n: Int) {
    private val z = Array.ofDim[Double](n - 1, 3) // 4)
    private var i = 0
    
    def apply(i: Int): Node = nodeFromArray(i, z(i))
    
    def add(node: Node): Unit = add(node.elem1, node.elem2, node.distance)
    
    def add(elem1: Int, elem2: Int, distance: Double): Unit = {
      z(i) = Array(elem1, elem2, distance)
      i += 1
    }
    
    def update(i: Int, node: Node): Unit = update(i, node.elem1, node.elem2, node.distance)
    
    def update(i: Int, elem1: Int, elem2: Int, distance: Double): Unit = {
      z(i) = Array(elem1, elem2, distance)
    }
 ...
 */
case class Hierarchy private (z: Array[Array[Double]]) extends Iterable[Hierarchy.Node] with IndexedSeq[Hierarchy.Node] {
  ...
  override def iterator: Iterator[Hierarchy.Node] = z.iterator.zipWithIndex.map((arr, i) => Hierarchy.nodeFromArray(i, arr))
