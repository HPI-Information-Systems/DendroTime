...
object ApproxDistanceWorkGenerator {
  ...
  def apply[T: Numeric : ClassTag](mapping: Map[T, Int],
    ...
    new ApproxDistanceWorkGenerator(queue, dists.n, direction)
  }
  ...
  private def createQueue[T: ClassTag](
    ...
    val work = Array.ofDim[(Double, T, T)]((dists.n * (dists.n - 1) / 2) - processedWork.size)
    var i = 0
    var k = 0
...
class ApproxDistanceWorkGenerator[T] private(queue: Array[(T, T)],
                                             n: Int,
                                             direction: ApproxDistanceWorkGenerator.Direction) extends WorkGenerator[T] {
  private var i = 0
  ...
  override def next(): (T, T) = {
    if !hasNext then
      throw new NoSuchElementException(
        s"ApproxDistanceWorkGenerator has no (more) work {i=$i, queue.length=${queue.length}}"
      )
    else
      val result =
