...
def main():
    ...
    print(f"Processing results from {len(experiments)} experiments ...", file=sys.stderr)
    entries = []
    for file in tqdm(experiments):
        ...
        for phase, runtime in exp_runtimes.itertuples(index=False):
            entries.append((exp.dataset, exp.distance, exp.linkage, exp.strategy, phase, runtime))
    df = pd.DataFrame(entries, columns=["dataset", "distance", "linkage", "strategy", "phase", "runtime"])
