...
class CuckooFilter[T](numberOfBuckets: Long, numberOfBitsPerItem: Int, private val table: UnsafeTable)
                     (using canGenerateHash: CanGenerateHashFrom[T]) extends Serializable {

  def this(numberOfBuckets: Long, numberOfBitsPerItem: Int)(using CanGenerateHashFrom[T]) =
...
object CuckooFilter {
  ...
  def apply[T](numberOfItems: Long)(using canGenerateHash: CanGenerateHashFrom[T]): CuckooFilter[T] =
  ...
  def optimalNumberOfBuckets(numberOfItems: Long): Long =
    var numberOfBuckets = upperPowerOf2(
      (numberOfItems + UnsafeTable16Bit.TagsPerBucket - 1) / UnsafeTable16Bit.TagsPerBucket
    )
  ...
  private val MaxAddAttempts = 500
