...
import de.hpi.fgis.bloomfilter.CanGenerate128HashFrom
...
class BloomFilter[T] private (val numberOfBits: Long, val numberOfHashes: Int, private val bits: UnsafeBitArray)
                             (using canGenerateHash: CanGenerate128HashFrom[T]) extends Serializable {
...
object BloomFilter {
  ...
  def apply[T](numberOfItems: Long, falsePositiveRate: Double)(using CanGenerate128HashFrom[T]): BloomFilter[T] =
  ...
  def readFrom[T](in: InputStream)(using CanGenerate128HashFrom[T]): BloomFilter[T] =
