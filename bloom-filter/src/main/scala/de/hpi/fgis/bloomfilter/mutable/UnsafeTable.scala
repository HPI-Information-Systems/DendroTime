...
trait UnsafeTable {
  ...
  protected def toSerializedForm(bytesPerBucket: Int, numberOfBuckets: Long): AnyRef =
    new UnsafeTable.SerializedForm(bytesPerBucket, numberOfBuckets, this)
...
object UnsafeTable {
  ...
  private class SerializedForm(
                                bytesPerBucket: Int, numberOfBuckets: Long, @transient var unsafeTable: UnsafeTable
...
class UnsafeTable8Bit(val numberOfBuckets: Long) extends UnsafeTable with Serializable {
  ...
  import UnsafeTable8Bit.*

  private[mutable] val ptr = unsafe.allocateMemory(bytesPerBucket * numberOfBuckets)
