...
import de.hpi.fgis.bloomfilter.mutable.{UnsafeTable, UnsafeTable16Bit, UnsafeTable8Bit}
...
import scala.language.{adhocExtensions, reflectiveCalls}
import scala.reflect.Selectable.reflectiveSelectable

class UnsafeTableSpec extends Properties("UnsafeTableSpec") with matchers.should.Matchers {
  ...
  class UnsafeTableCommands extends Commands {
    ...
    case class ReadTag(index: Long, tagIndex: Int, tag: Byte) extends SuccessCommand {
      ...
      def run(sut: Sut): Boolean =
        sut.synchronized(sut.readTag(index, tagIndex) == tag)
  ...
  class UnsafeTableInsertFindCommands extends Commands {
    ...
    override def genInitialState: Gen[State] =
      Gen
        .chooseNum[Long](1, /*Int.MaxValue * 2L*/ 1000)
        .map(State(_, 0, Map.empty))

    override def newSut(state: State): Sut = new UnsafeTable8Bit(state.numberOfBuckets)
    ...
    case class Insert(index: Long, tag: Byte) extends UnitCommand {
      ...
      def nextState(state: State): State =
        val nextBucketsPopulation = state.bucketsPopulation.updated(
          index,
          prevBucketPopulation(state) + 1
        )
        state.copy(
          addedItems = state.addedItems + 1,
          bucketsPopulation = nextBucketsPopulation
        )

      def prevBucketPopulation(state: State): Int =
        state.bucketsPopulation.getOrElse(index, 0)
  ...
  def serializationProp(mkTable: Long => UnsafeTableEx): Prop =
    ...
    Prop.forAllNoShrink(gen) { case (numBuckets, tags) =>
      ...
      try
        ...
        deserialized should have(Symbol("class")(sut.getClass))
        val sut2 = deserialized.asInstanceOf[UnsafeTableEx]
        sut2.ptr should not be 0
        sut2.ptr should not equal sut.ptr
        try
          for {
  ...
  property("UnsafeTable8Bit supports java serialization") = serializationProp(
    new UnsafeTable8Bit(_)
  )
